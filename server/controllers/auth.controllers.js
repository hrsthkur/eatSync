...
export const signup = async(req,res)=>{
    try {
        ...
        if(mobile.length < 10){
             return res.status(400).json({message: "Mobile should have atleat 10 digits"})
...
export const sendOtp = async(req,res) =>{
    try {
        ...
        return res.status(500).json(`Erroring in sending Otp ${error}`)
    }
...
export const verifyOtp = async(req,res) => {
    try {
        ...
        const user = await User.findOne({email});
        if(!user || user.resetOtp != otp || user.otpExpires < Date.now()){ return res.status(400).json("Invalid or expired otp")}
